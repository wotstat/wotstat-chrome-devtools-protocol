...
class WebSocket(object):  # pylint: disable=too-many-instance-attributes
    ...
    def handle_http_request(self, request):
        ...
        Args:
            request: an HTTPRequest instance (has .command, .path, .headers) and an
                     extra attribute .body (bytes) we attach before calling you.

        Returns:
            (status_code: int, headers: list[(k, v)], body: bytes|str|None)

        Default: 404 Not Found with a small text payload.
        """
        body = b'Not Found'
        return (404, [('Content-Type', 'text/plain; charset=utf-8')], body)

    def _send_http_response(self, status, headers, body, keep_alive=False):
        if body is None:
            body = b''
        if _check_unicode(body):
            body = body.encode('utf-8')

        reason = self._HTTP_STATUS_REASONS.get(status, 'OK')
        start = 'HTTP/1.1 %d %s\r\n' % (status, reason)
        # normalize headers into a dict-like sequence, but allow duplicates (e.g., Set-Cookie)
        header_lines = []
        has_len = False
        has_conn = False
        ...
        if not has_len:
            header_lines.append('Content-Length: %d\r\n' % len(body))
        if not has_conn:
            header_lines.append('Connection: %s\r\n' % ('keep-alive' if keep_alive else 'close'))

        resp = (start + ''.join(header_lines) + '\r\n').encode('ascii') + body
        # best-effort: send all then return leftover if any (we force send_all=True)
        self._send_buffer(resp, send_all=True)
    ...
    def _handle_data(self):
        # do the HTTP header and handshake OR serve plain HTTP
        if self.handshaked is False:
            ...
            # Do we have at least the full header?
            header_end = self.headerbuffer.find(b'\r\n\r\n')
            if header_end == -1:
                # keep reading more header bytes
                self.headertoread = min(4096, self.maxheader - len(self.headerbuffer))
                return

            # Parse the request (line + headers). We pass the entire buffer; the parser
            # only consumes request line/headers — we'll extract body ourselves.
            if self.request is None:
                self.request = HTTPRequest(self.headerbuffer)

            # Determine if it's a WebSocket upgrade
            upgrade = self.request.headers.get('Upgrade', '')
            is_ws = isinstance(upgrade, str) and upgrade.lower() == 'websocket'

            # Compute required total bytes (header + optional body)
            content_len = int(self.request.headers.get('Content-Length', '0') or '0')
            total_needed = header_end + 4 + content_len

            # If we still haven't received the full HTTP body, read more
            if len(self.headerbuffer) < total_needed:
                self.headertoread = min(total_needed - len(self.headerbuffer), 65536)
                return

            # Slice out the body (may be empty). Attach it to the request object.
            body_bytes = self.headerbuffer[header_end + 4: total_needed]
            # Attach .body for convenience
            try:
                setattr(self.request, 'body', bytes(body_bytes))
            except Exception:
                self.request.body = bytes(body_bytes)
            ...
            if is_ws:
                ...
                try:
                    ...
                    # It's an attempted upgrade but not valid -> 426 Upgrade Required
                ...
                try:
                    status, headers, resp_body = self.handle_http_request(self.request)
                    self._send_http_response(status, headers, resp_body, keep_alive=False)
                except Exception as e:
                    # Safeguard: send 500 if handler raises
                    try:
                        self._send_http_response(
                            500,
                            [('Content-Type', 'text/plain; charset=utf-8')],
                            u'Internal Server Error: {}'.format(e)
                        )
                    except Exception:
                        pass
                finally:
                    # Close the socket and bubble up so the server removes the client
                    try:
                        self.client.close()
                    finally:
                        raise Exception('http request handled, connection closed')
        else:
            # --- existing frame-level read path (unchanged) ---
            try:
    ...
    def send_message_text(self, data):
        """
          Send websocket data frame to the client as Text.
        """
        self._send_message(False, TEXT, data.encode('utf-8'))
